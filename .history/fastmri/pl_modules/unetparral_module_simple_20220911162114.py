...
from fastmri.models.unet import Unet_add_channel_attention_add_res_addcat, Unet_Kimgsampling
...
class NormUnet(nn.Module):
    def __init__(self, inp, out, chans, num_pools):
        super().__init__()
        self.unet = Unet_add_channel_attention_add_res_addcat(inp, out, chans, num_pools)
        # self.unet = Unet_ynet(2, 2, chans, num_pools)
    ...
    def norm(self, x):
        ...
        # mean = x.mean(dim=2).view(b, c, 1, 1, 1).contiguous().view(b, c, 1, 1)
        # std = x.std(dim=2).view(b, c, 1, 1, 1).contiguous().view(b, c, 1, 1)
        # x = x.view(b, c, h, w)
        # return (x - mean) / std, mean, std        
...
class NormKUnet(nn.Module):
    def __init__(self, inp, out, chans, num_pools):
        super().__init__()
        self.unet = Unet_Kimgsampling(inp, out, chans, num_pools)
...
class DualUNetSimpleModule(MriModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(7):
            ...
            self.dc_weight_k.append(nn.Parameter(torch.ones(1)))
            self.fuse_weight_i.append(nn.Parameter(torch.ones(1)))
            self.dtrans_i.append(NormUnet(2, 2, 32, 3))
            self.dtrans_k.append(NormKUnet(2, 2, 12, 3))

        self.addynet_1 = NormUnet(2, 2, 32, 3)
        self.addynet_2 = NormUnet(2, 2, 32, 3)
        self.q_1 = nn.Parameter(torch.ones(1))
        self.q_2 = nn.Parameter(torch.ones(1))
        self.loss = SSIMLoss()
    ...
    def batched_mask_center_high(self, x: torch.Tensor, mask_from_left: torch.Tensor, mask_to_left: torch.Tensor,mask_from_right: torch.Tensor, mask_to_right: torch.Tensor,
) -> torch.Tensor:
        """
    ...
    def forward(self, image, mask, masked_kspace,image_pocs, masked_kspace_pocs):
        ...
        masked_kspace_high = self.batched_mask_center_high(
            masked_kspace.permute(0,3,1,2), pad - pad, pad, pad + num_low_freqs, 2 * pad + num_low_freqs
        ).permute(0,2,3,1)
        
        masked_kspace_high_pocs = self.batched_mask_center_high(
            masked_kspace_pocs, pad - pad, pad, pad + num_low_freqs, 2 * pad + num_low_freqs
        ).permute(0,2,3,1)
        ...
        for li, lk, wi, wk, wfi in zip(self.dtrans_i, self.dtrans_k, self.dc_weight_i, self.dc_weight_k, self.fuse_weight_i):
            ...
            if(i==0):
                ...
                image = li(image, image_high_copy) + image
            image_k = fastmri.fft2c(image.permute(0,2,3,1))
            image_k_dc = image_k - torch.where(mask, image_k - masked_kspace, zero) * wi
            kspace = image_k_dc.permute(0,3,1,2)
            ...
            # image = torch.cat([image, image_pocs], dim=1)
            # kspace_k_dc_high = self.batched_mask_center_high(
            # kspace, pad - pad, pad, pad + num_low_freqs, 2 * pad + num_low_freqs
            # ).permute(0,2,3,1)
            # image_high = fastmri.ifft2c(kspace_k_dc_high).permute(0,3,1,2)
            # image_high = torch.cat([image_high, image_high_pocs], dim = 1)
        image = self.addynet_1(image, image_high_copy) + image
        image_k = fastmri.fft2c(image.permute(0,2,3,1))
        image_k_dc = image_k - torch.where(mask, image_k - masked_kspace, zero) * self.q_1
        image = fastmri.ifft2c(image_k_dc).permute(0,3,1,2)    

        image = self.addynet_2(image, image_high_copy) + image
        image_k = fastmri.fft2c(image.permute(0,2,3,1))
        image_k_dc = image_k - torch.where(mask, image_k - masked_kspace, zero) * self.q_2

        unet_out_abs = fastmri.complex_abs(fastmri.ifft2c(image_k_dc))
        # unet_out_abs = fastmri.complex_abs(fastmri.ifft2c(kspace.permute(0,2,3,1)))
        return unet_out_abs

    def training_step(self, batch, batch_idx):
        image, target, mean, std, fname, slice_num, max_value, mask, masked_kspace, image_pocs, masked_kspace_pocs = batch
        output = self(image, mask, masked_kspace,image_pocs, masked_kspace_pocs)
