...
class UnetData_2channel_allresolution_Transform_visual:
    ...
    def __call__(
        ...
        under_image = fastmri.ifft2c(masked_kspace)

        masked_kspace_copy = masked_kspace.permute(2, 0, 1)
        ...
        image_pocs = copy.deepcopy(under_image)

        for i in range(5):
            image_pocs = torch.exp(1j * (torch.angle(masked_kspace_center_img))) * torch.abs(image_pocs)
            kspace_pocs = fastmri.fft2c(image_pocs.float())
            kspace_pocs = kspace_pocs.permute(2, 0, 1)  # 2,640,368
            kspace_pocs = (torch.rot90(kspace_pocs, k=2, dims=[1, 2])).permute(2,0,1)

            kspace_pocs[:, :, 0:start] = masked_kspace_copy[:, :, 0:start]
            kspace_pocs = kspace_pocs * (masked_kspace_copy == 0) + masked_kspace_copy
            image_pocs = fastmri.ifft2c(kspace_pocs.permute(1, 2, 0))
        ...
        return under_image.permute(2,0,1), target, 0, 0, fname, slice_num, max_value, mask.byte(), masked_kspace, image_pocs.permute(2,0,1), kspace_pocs
