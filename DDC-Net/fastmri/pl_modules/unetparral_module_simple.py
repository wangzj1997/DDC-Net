...
class NormInet(nn.Module):
    def __init__(self, inp, out, chans, num_pools):
        super().__init__()
        self.inet = Inet(inp, out, chans, num_pools)
...
class NormKnet(nn.Module):
    def __init__(self, inp, out, chans, num_pools):
        super().__init__()
        self.knet = Knet(inp, out, chans, num_pools)
...
class DualUNetSimpleModule(MriModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(8):
            ...
            self.dtrans_k.append(NormKnet(4, 2, 8, 3))

        self.loss = SSIMLoss()


    def forward(self, image, mask, masked_kspace, image_pocs, kspace_pocs):

        zero = torch.zeros(1, 1, 1, 1).to(masked_kspace)

        pad, num_low_freqs = self.get_pad_and_num_low_freqs(mask)

        masked_kspace_high = self.batched_mask_center_high(masked_kspace.permute(0,3,1,2), pad - pad, pad, pad + num_low_freqs, 2 * pad + num_low_freqs).permute(0,2,3,1)
        masked_kspace_high_pocs = self.batched_mask_center_high(kspace_pocs, pad - pad, pad, pad + num_low_freqs, 2 * pad + num_low_freqs).permute(0,2,3,1)
        ...
        kspace = torch.cat([masked_kspace.permute(0,3,1,2), kspace_pocs], dim=1)
        image = torch.cat([image, image_pocs], dim=1)
        image_high = torch.cat([image_high, image_high_pocs], dim = 1)

        for li, lk, wi, wk in zip(self.dtrans_i, self.dtrans_k, self.dc_weight_i, self.dc_weight_k):
            ...
            kspace = torch.cat([kspace, kspace_pocs], dim=1)
            kspace = lk(kspace)
    ...
    def training_step(self, batch, batch_idx):
        image, target, mean, std, fname, slice_num, max_value, mask, masked_kspace, image_pocs, kspace_pocs = batch
        output = self(image, mask, masked_kspace, image_pocs, kspace_pocs)
